...
export class ShapeChallenge {
  ...
  generateCirclePath(centerX, centerY, centerZ, radius, points) {
    console.log('🔍 CIRCLE SETTINGS DEBUG:');
    console.log('  Center X:', centerX);
    console.log('  Center Y:', centerY);
    console.log('  Center Z:', centerZ);
    console.log('  Radius:', radius);
    console.log('  Points:', points);
    console.log('  Speed:', this.challengeConfig.speed);
    console.log('  Full Config:', JSON.stringify(this.challengeConfig, null, 2));
    ...
    console.log('  Generated', this.targetDrawing.length, 'points for circle');
    console.log('  First point:', this.targetDrawing[0]);
    console.log('  Last point:', this.targetDrawing[this.targetDrawing.length - 1]);
  }
  ...
  calculateDotConnectionAccuracy() {
    ...
    this.targetDrawing.forEach(point => {
      const gridX = Math.floor(((point.x - targetBounds.minX) / (targetBounds.maxX - targetBounds.minX)) * (gridSize - 1));
      const gridY = Math.floor(((point.y - targetBounds.minY) / (targetBounds.maxY - targetBounds.minY)) * (gridSize - 1));
  ...
  calculateTriangleAngles(vertices) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      const angle = Math.acos((a ** 2 + b ** 2 - c ** 2) / (2 * a * b)) * (180 / Math.PI);
      angles.push(angle);
    }
    return angles;
  }
  ...
  printCurrentSettings() {
    ...
    console.log('  Active Challenge:', this.currentChallenge ? this.currentChallenge.name : 'None');
    console.log('  Config:', JSON.stringify(this.challengeConfig, null, 2));
    console.log('  Target Drawing Points:', this.targetDrawing.length);
    if (this.targetDrawing.length > 0) {
      console.log('  First Target Point:', this.targetDrawing[0]);
      console.log('  Last Target Point:', this.targetDrawing[this.targetDrawing.length - 1]);
    }
    console.log('  User Drawing Points:', this.userDrawing.length);
    console.log('  Is Target Showing:', this.isShowingTarget);
    console.log('  Auto Detection Run:', this.autoDetectionRun || 'N/A (not in PaintingSystem)');
  }
} 